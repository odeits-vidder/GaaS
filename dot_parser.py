...
def push_top_graph_stmt(stmt, loc, toks):
    attrs = {}
    g = None

    for elmnt in toks:

        if isinstance(elmnt, (ParseResults, tuple, list)) and len(elmnt) == 1 and isinstance(elmnt[0], type(stmt)):
            elmnt = elmnt[0]

        if elmnt == 'strict':
            attrs['strict'] = True

        elif elmnt in ['graph', 'digraph']:

            attrs = {}

            g = pydot.Dot(graph_type=elmnt, **attrs)
            attrs['type'] = elmnt

            top_graphs.append(g)

        elif isinstance(elmnt, type(stmt)):
            g.set_name(elmnt)

        elif isinstance(elmnt, pydot.Subgraph):

            g.obj_dict['attributes'].update(elmnt.obj_dict['attributes'])
            g.obj_dict['edges'].update(elmnt.obj_dict['edges'])
            g.obj_dict['nodes'].update(elmnt.obj_dict['nodes'])
            g.obj_dict['subgraphs'].update(elmnt.obj_dict['subgraphs'])

            g.set_parent_graph(g)

        elif isinstance(elmnt, P_AttrList):
            attrs.update(elmnt.attrs)

        elif isinstance(elmnt, (ParseResults, list)):
            add_elements(g, elmnt)

        else:
            raise ValueError("Unknown element statement: %r " % elmnt)
